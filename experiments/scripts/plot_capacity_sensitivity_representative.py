#!/usr/bin/env python3
"""Aggregate and plot the Footscray representative capacity sensitivity."""
...
DEFAULT_RESULTS = Path(
    "experiments/results/footscray/capacity_sensitivity_representative_footscray"
)
FLEET_ORDER = ["balanced", "vmt_oriented", "low_emission", "all_car"]
FLEET_LABELS = {
    "balanced": "Balanced",
    "vmt_oriented": "VMT-oriented",
    "low_emission": "Low-emission S50/M50",
    "all_car": "All-car",
}
...
def reduction_pct(
    ...
    if not math.isnan(change):
        ...
        return -change
    total = number(comparison.get(f"system_total_{metric}_km"), metrics.get(f"system_total_{metric}_km")) if metric == "vmt" else number(
        comparison.get(f"system_total_{metric}_kwh" if metric == "energy" else f"system_total_{metric}_kg"),
        metrics.get(f"system_total_{metric}_kwh" if metric == "energy" else f"system_total_{metric}_kg"),
    )
    baseline_total = number(
        baseline.get(f"baseline_total_{metric}_km" if metric == "vmt" else f"baseline_total_{metric}_kwh" if metric == "energy" else f"baseline_total_{metric}_kg"),
        baseline.get(f"total_{metric}_km" if metric == "vmt" else f"total_{metric}_kwh" if metric == "energy" else f"total_{metric}_kg"),
    )
...
def extract_run(metrics_path: Path) -> dict[str, Any]:
    ...
    metadata = config.get("capacity_metadata", {})
    if not metadata:
        raise ValueError(f"Missing capacity_metadata in {run_dir / 'config.json'}")

    served = number(comparison.get("served_commuters"), metrics.get("served_commuters"))
    total = number(comparison.get("total_commuters"), metrics.get("total_commuters"))
    ...
    row = {
        **metadata,
        ...
        "fallback_private_cars": number(
            ...
            metrics.get("fallback_private_cars"),
            number(metrics.get("late_deliveries"), 0) + number(metrics.get("unserved_commuters"), 0),
        ),
        ...
        "system_total_vmt_km": number(comparison.get("system_total_vmt_km"), metrics.get("system_total_vmt_km")),
        "system_total_energy_kwh": number(comparison.get("system_total_energy_kwh"), metrics.get("system_total_energy_kwh")),
        "system_total_co2_kg": number(comparison.get("system_total_co2_kg"), metrics.get("system_total_co2_kg")),
        "avg_passengers_per_trip": number(comparison.get("avg_passengers_per_trip"), metrics.get("avg_passengers_per_trip")),
    }
    for metric in OPTIONAL_METRICS:
        row.setdefault(metric, number(comparison.get(metric), metrics.get(metric), baseline.get(metric)))
    return row
...
def summarize(runs: pd.DataFrame) -> pd.DataFrame:
    ...
    result["_rank"] = result["fleet_name"].map(ranks)
    return result.sort_values(["_rank", "actual_total_seats"]).drop(columns="_rank")
...
def plot(summary: pd.DataFrame, out_dir: Path) -> list[Path]:
    ...
    panels = [
        ...
        ("system_vmt_reduction_pct", "System VMT reduction (%)"),
        ("system_co2_reduction_pct", "System CO$_2$ reduction (%)"),
    ]
    fig, axes = plt.subplots(1, 3, figsize=(9.2, 3.2), sharex=True)
    for ax, (metric, ylabel) in zip(axes, panels):
        for fleet in FLEET_ORDER:
            part = summary[summary["fleet_name"] == fleet].sort_values("actual_total_seats")
            if part.empty or part[f"{metric}_mean"].isna().all():
                continue
            ax.errorbar(
                part["actual_total_seats"], part[f"{metric}_mean"],
                yerr=part[f"{metric}_std"], color=COLORS[fleet], marker=MARKERS[fleet],
                capsize=3, label=FLEET_LABELS[fleet],
            )
        ax.axvline(80, color="0.4", linestyle="--", linewidth=1)
        ax.set_xlabel("Actual fleet capacity (seats)")
        ax.set_ylabel(ylabel)
        ax.grid(True, linestyle=":", linewidth=0.7)
    handles, labels = axes[0].get_legend_handles_labels()
    fig.legend(handles, labels, loc="lower center", ncol=4, frameon=False)
    fig.subplots_adjust(bottom=0.28, wspace=0.38)
    paths = [out_dir / "capacity_sensitivity_response.pdf", out_dir / "capacity_sensitivity_response.png"]
...
def main() -> None:
    ...
    runs_path = out_dir / "capacity_sensitivity_runs.csv"
    summary_path = out_dir / "capacity_sensitivity_summary.csv"
